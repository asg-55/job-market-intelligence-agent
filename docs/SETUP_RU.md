...
В Swagger вызовите `POST /vacancies/{id}/resume-advice` и передайте текст текущего резюме:

```json
{"resume_text": "Текст резюме длиной не менее 50 символов", "language": "ru"}
```

В ответе будут фактологически подтверждённые пункты, изменения структуры, навыки для акцента
и честно отмеченные пробелы. Основное резюме не изменяется. Исходный текст не сохраняется в
SQLite: для связи с версией документа записывается только SHA-256 отпечаток.
